...
heuristic = (program, _) -> sum([
    interp(property, Dict(:_arg_1 => interp(program, Dict()))) ? -score : score
    for (property, score) in property_scores
])

iterator = BeamIterator(int_grammar, :Int,
    beam_size = 10,
    max_extension_depth = 2,
