...
function encode_state(state; use_pointer=false)
    if state isa String
        s = state
    else
        s = state.str
    ...
    # idxs = [2 * Int(c) for c in collect(s)]
    ...
    bit_vectors = [digits(Int(b), base=2, pad=9) |> reverse for b in codeunits(s)]

    if length(bit_vectors) > 0
        if !isnothing(state.pointer) && use_pointer
            bit_vectors[state.pointer][1] = 1
        end
    else
        bit_vectors = [[0,0,0,0,0,0,0,0,0]]
    end

    # return Flux.onehotbatch(idxs, 1:2^8)
    return hcat(bit_vectors...)
end
...
function get_model(;triplets, embed_dim, hidden_dim, learning_rate)
    amount_of_properties = io_string_properties_size
    triplet_loss_margin = .1

    encoder = Chain(
        Parallel(hcat, Dense(9 => embed_dim)),
        GRU(embed_dim => hidden_dim),
        x -> x[:, end],
    )

    encoder_combiner = Parallel((x,y) -> sum((x.-y).^2), encoder, encoder)

    property_encoder = identity#Dense(amount_of_properties => 8, tanh)

    # model = Chain(
    #     Parallel(vcat, encoder_combiner, property_encoder),
    #     Dense(1 + amount_of_properties => 1, bias=false),
    #     x -> x[1]
    # )

    model = cu(Chain(
        Parallel((x,y) -> x, encoder_combiner, property_encoder),
        x -> x[1]
    ))

    function loss(model, anchors, positives, negatives)
        triplet_loss(d_ap, d_an) = max(0, d_ap - d_an + triplet_loss_margin)

        encodings_a = encode_state.(anchors)
        encodings_p = encode_state.(positives)
        encodings_n = encode_state.(negatives)

        signs_ap = io_string_properties.(anchors, positives)
        signs_an = io_string_properties.(anchors, negatives)

        values_ap = model.(zip(encodings_a, encodings_p), signs_ap)
        values_an = model.(zip(encodings_a, encodings_n), signs_an)
...
function model_heuristic(model, input_states, output_states)
    ...
    encodings_in = encode_state.(input_states)
    encodings_out = encode_state.(output_states)

    signs = io_string_properties.(input_states, output_states)

    values_io = model.(zip(encodings_in, encodings_out), signs)
...
m = execute_experiment(;
    ...
    problem_ids = [15],
    example_ids = 1:5,
    amount_of_programs_exploration = 10,
...
# result = (0.7999999999999999, 6.0, 1.6035674514745464)
