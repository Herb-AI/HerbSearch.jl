...
function search(;
    ...
    for iteration in 1:max_iterations
        ...
        println("\nIteration:\t $iteration\t\t Best cost:  $(iterator.beam[1].cost)\t\t Best outputs:  $(beam_outputs[1])")

        best_property = nothing
        best_property_index = 0
        best_property_representation = nothing
        best_property_score = -Inf
        best_possible_score = count(o != ot for outputs in beam_outputs for (o, ot) in zip(outputs, target_outputs))
        ...
        for (property_index, (property, representation)) in enumerate(properties)
            ...
            for beam_entry_outputs in beam_outputs
                for (input, beam_entry_output, target_value, target_output) in zip(inputs, beam_entry_outputs, target_values, target_outputs)
                    if beam_entry_output == target_output
                        continue
                    end
        ...
        println("Iteration:\t $iteration\t\t Best score: $best_property_score/$best_possible_score\t\t Best property: $best_property_representation")
