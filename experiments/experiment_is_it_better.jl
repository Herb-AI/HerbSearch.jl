...
function experiment_speedup_main(
    ...
    open(res_file_path, "w") do io
        redirect_stdout(io) do
            ...
            compress_set, _ = take_mth_fraction(problem_grammar_pairs, 5, using_mth)
            ...
            synthesize_and_time([p.problem for p in problem_grammar_pairs],
             optimiszed_grammar,
             benchmark,
             problem_name)
        end
...
function synthesize_and_time(problems::Vector{<:Problem},
    ...
    for problem in problems
        ...
        program, iter_count = synth_program(spec, grammar, iterator, benchmark, problem_name)
        tree_size = get_size_of_a_tree(program)
        push!(tree_sizes, tree_size)
        push!(iter_counts, iter_count)
        println("iterations $(iter_count) tree_size $(tree_size)")
    end
    println("Total time (ms) $(Millisecond(Time(now())).value-time_all)")
    println("num of iterations:\n$(join(iter_counts, " "))")
    println("tree_sizes:\n$(join(tree_sizes, " "))")
end
...
timestamp = Dates.format(now(), "yyyy-mm-dd_HH-MM-SS")
    ...
    open(res_file_path, "w") do io
        redirect_stdout(io) do
            ...
            problem_grammar_pairs = get_all_problem_grammar_pairs(benchmark)
            grammar = problem_grammar_pairs[1].grammar
            synthesize_and_time([p.problem for p in problem_grammar_pairs], grammar, benchmark, problem_name)
        end
    end
end
