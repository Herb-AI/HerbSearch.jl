...
struct CostPriorityBank #@TODO Type this properly
    buckets::DefaultDict{Float64,DefaultDict{Symbol,Vector{AbstractRuleNode}}}
    frontier::PriorityQueue{AccessAddress,Float64}
    # seen::Set{UInt64}  # de-dup on structural hash of ASTs (optional but useful)

    function CostPriorityBank()
        new(
            DefaultDict{Float64,DefaultDict{Symbol,Vector{AbstractRuleNode}}}(
                () -> DefaultDict{Symbol,Vector{AbstractRuleNode}}(() -> AbstractRuleNode[])
            ),
            PriorityQueue{AccessAddress,Float64}()
        )
    end
end

# Accessors required by your generic helpers
get_measures(b::CostPriorityBank) = keys(b.buckets)
get_types(b::CostPriorityBank, m) = keys(b.buckets[m])
get_programs(b::CostPriorityBank, m, T) = b.buckets[m][T]
retrieve(b::CostPriorityBank, a::AccessAddress) = get_programs(b, a.measure, a.type)[a.index]

function cheapest_address_of_type(bank::CostPriorityBank, T::Symbol)
    # Find the smallest cost bucket that has at least one program of type T
    best_cost = nothing
    for c in get_measures(bank)
        if any(isa(p, AbstractRuleNode) for p in get_programs(bank, c, T))
            best_cost = isnothing(best_cost) ? c : min(best_cost, c)
        end
    end
    isnothing(best_cost) && return nothing
    idx = 1
    AccessAddress(best_cost, T, idx, 1, 1)  # @TODO check size and depth properly
end

function enqueue!(bank::CostPriorityBank, addr::AccessAddress, priority::Float64)
    bank.frontier[addr] = priority
end

function dequeue_best!(bank::CostPriorityBank)
    isempty(bank.frontier) && return nothing
    dequeue!(bank.frontier)
end

@programiterator CostBasedBottomUpIterator(
    bank=CostPriorityBank(),
...
function populate_bank!(iter::CostBasedBottomUpIterator)
    ...
    accs = AccessAddress[]
    for t in unique(g.types)
        terminal_mask = g.isterminal .& g.domains[t]
        if any(terminal_mask)
            uhole = UniformHole(terminal_mask, [])
            cost  = calc_measure(iter, uhole)  # cost = min cost inside hole
            push!(get_programs(bank, cost, t), uhole)
            idx = length(get_programs(bank, cost, t))
            acc = AccessAddress(cost, t, idx, 1, 1)
            enqueue!(bank, acc, cost)
            push!(accs, acc)
        end
    end
    accs
...
function add_to_bank!(iter::CostBasedBottomUpIterator,
                      combination_address::CombineAddress,
                      constructed_program::AbstractRuleNode)
    bank = get_bank(iter)
    grammar = get_grammar(get_solver(iter))

    program_type = return_type(grammar, constructed_program)
    total_cost = calc_measure(iter, combination_address)  # cost(op) + sum(child costs)

    total_cost > get_measure_limit(iter) && return false

    push!(get_programs(bank, total_cost, program_type), constructed_program)
    index_in_bucket = length(get_programs(bank, total_cost, program_type))
    address = AccessAddress(total_cost, program_type, index_in_bucket, 1, 1)
    enqueue!(bank, address, total_cost)

    return true
end

function one_step_refinements(iter::CostBasedBottomUpIterator, best::AccessAddress)
    g = get_grammar(iter.solver)
    bank = get_bank(iter)
    terminals = g.isterminal
    nonterminals = .~terminals
    shapes = UniformHole.(partition(Hole(nonterminals), g), ([],))  # operators

    best_type = get_return_type(best)
    results = CombineAddress[]

    for shape in shapes
        child_types = Tuple(g.childtypes[findfirst(shape.domain)])
        n = length(child_types)

        # Place `best` into each compatible child position; fill others with current cheapest of needed types
        for j in 1:n
            child_types[j] == best_type || continue

            # Build child address tuple
            addrs = Vector{AccessAddress}(undef, n)
            feasible = true
            for k in 1:n
                if k == j
                    addrs[k] = best
                else
                    address = cheapest_address_of_type(bank, child_types[k])
                    if address === nothing
                        feasible = false
                        break
                    end
            ...
            comb = CombineAddress(shape, Tuple(addrs))
            total_cost = calc_measure(iter, comb)
...
function combine(iter::CostBasedBottomUpIterator, state)
    bank = get_bank(iter)
    # Stop if frontier empty
    best = dequeue_best!(bank)
    if best === nothing
        return nothing, nothing
    end

    # Prune if the best itself already exceeds the max cost
    if get_measure(best) > get_measure_limit(iter)
        return nothing, nothing
    end

    # Generate single-step refinements around the best
    addresses = one_step_refinements(iter, best)

    # If there are no refinements for this best item, try again (pop next)
