...
function processdecl(mod::Module, mut::Bool, decl::Expr, super = nothing)
    @match decl begin
        Expr(:call, name::Symbol, extrafields...) => begin
            ...
            RESERVERD_ARG_NAMES = [
                :solver, :start_symbol, :initial_node, :grammar, :max_depth, :max_size]
            ...
            begin
                ...
                function $(escaped_name)(
                        $(notkwargs...); solver::Solver, max_size = nothing,
                        max_depth = nothing, $(kwargs_fields...))
                ...
                function $(escaped_name)(
                        grammar::AbstractGrammar, start_symbol::Symbol, $(notkwargs...);
                        max_size = typemax(Int), max_depth = typemax(Int), $(kwargs_fields...))
                    return $(escaped_name)(
                        GenericSolver(grammar, start_symbol, max_size = max_size,
                            max_depth = max_depth),
                        $(field_names...))
                end

                # solver with grammar and initial rulenode to start with
                function $(escaped_name)(grammar::AbstractGrammar,
                        initial_node::AbstractRuleNode, $(notkwargs...);
                        max_size = typemax(Int), max_depth = typemax(Int), $(kwargs_fields...))
                    return $(escaped_name)(
                        GenericSolver(grammar, initial_node, max_size = max_size,
                            max_depth = max_depth),
                        $(field_names...))
                end
            end
            ...
            head = Expr(:(<:), name,
                isnothing(super) ? :(HerbSearch.ProgramIterator) : :($mod.$super))
...
x::Int = 3 -> x
"""
extract_name_from_argument(ex) = @match ex begin
    Expr(:(::), name, type) => name
    name::Symbol => name
