...
function Base.iterate(iter::StochasticSearchIterator, iterator_state::IteratorState)
    ...
    improved_program = try_improve_program!(
        iter, possible_programs, neighbourhood_node_location, new_temperature, current_cost)
...
function try_improve_program!(iter::StochasticSearchIterator, possible_programs,
        neighbourhood_node_location::NodeLoc, new_temperature, current_cost)
...
"""
function _calculate_cost(program::Union{RuleNode, StateHole},
    ...
    for example in filter(e -> e isa IOExample, spec)
...
"""
calculate_cost(iter::T, program::Union{RuleNode, StateHole}) where {T <: StochasticSearchIterator} = _calculate_cost(
    program, iter.cost_function, iter.spec,
    get_grammar(iter.solver), iter.evaluation_function)

function neighbourhood(
        iter::T, current_program::RuleNode) where {T <: StochasticSearchIterator}
    constructNeighbourhood(current_program, get_grammar(iter.solver))
end
...
function propose(
        iter::MHSearchIterator, path::Vector{Int}, dict::Union{Nothing, Dict{String, Any}})
    random_fill_propose(iter.solver, path, dict)
end

function temperature(::MHSearchIterator, current_temperature::Real)
    const_temperature(current_temperature)
end

function accept(::MHSearchIterator, current_cost::Real, next_cost::Real, temperature::Real)
    probabilistic_accept(current_cost, next_cost, temperature)
end
...
function propose(iter::VLSNSearchIterator, path::Vector{Int},
        dict::Union{Nothing, Dict{String, Any}})
    enumerate_neighbours_propose(iter.vlsn_neighbourhood_depth)(iter.solver, path, dict)
end

function temperature(::VLSNSearchIterator, current_temperature::Real)
    const_temperature(current_temperature)
end

function accept(
        ::VLSNSearchIterator, current_cost::Real, next_cost::Real, temperature::Real)
    best_accept(current_cost, next_cost, temperature)
end
...
function propose(
        iter::SASearchIterator, path::Vector{Int}, dict::Union{Nothing, Dict{String, Any}})
    random_fill_propose(iter.solver, path, dict)
end

function temperature(iter::SASearchIterator, current_temperature::Real)
    decreasing_temperature(iter.temperature_decreasing_factor)(current_temperature)
end

function accept(::SASearchIterator, current_cost::Real, next_cost::Real, temperature::Real)
    probabilistic_accept_with_temperature(current_cost, next_cost, temperature)
end
