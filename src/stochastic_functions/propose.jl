...
"""
    random_fill_propose(solver::Solver, path::Vector{Integer}, dict::Union{Nothing,Dict{String,Any}}, nr_random=5)

Returns a list with only one proposed, completely random, subprogram.
# Arguments
- `solver::solver`: solver
- `path::Vector{Integer}`: path to the location to be filled.
...
"""
function random_fill_propose(solver::Solver, path::Vector{<:Integer}, dict::Union{Nothing,Dict{String,Any}}, nr_random=1)
    return Iterators.take(RandomSearchIterator(solver, path), nr_random)
end 

"""
    enumerate_neighbours_propose(enumeration_depth::Integer64)

The return function is a function that produces a list with all the subprograms with depth at most `enumeration_depth`.
"""
function enumerate_neighbours_propose(enumeration_depth::Integer)
    return (solver::Solver, path::Vector{Integer}, dict::Union{Nothing,Dict{String,Any}}) -> begin
