...
"""
function hole_heuristic(::TopDownIterator, node::AbstractRuleNode,
...
function Base.iterate(iter::TopDownIterator)
    ...
    if isfeasible(solver)
        enqueue!(pq, get_state(solver),
            priority_function(iter, get_grammar(solver), get_tree(solver), 0, false))
    end
...
"""
function Base.iterate(iter::TopDownIterator,
        tup::Tuple{Vector{<:AbstractRuleNode}, DataStructures.PriorityQueue})
        ...
        if item isa UniformIterator
            ...
            if !isnothing(solution)
                enqueue!(pq,
                    uniform_iterator,
                    priority_function(
                        iter, get_grammar(solver), solution, priority_value, true))
                return (solution, pq)
            end
        elseif item isa SolverState
            ...
            if hole_res ≡ already_complete
                ...
                if solver.use_uniformsolver
                    uniform_solver = UniformSolver(get_grammar(solver), get_tree(solver),
                    ...
                    fixed_shaped_iter = FixedShapedIterator(
                        get_grammar(solver), :StartingSymbolIsIgnored, solver = solver)
            ...
            elseif hole_res isa HoleReference
                ...
                for (i, domain) in enumerate(partitioned_domains)
                    ...
                    if isfeasible(solver)
                        enqueue!(pq,
                            get_state(solver),
                            priority_function(iter, get_grammar(solver),
                                get_tree(solver), priority_value, false))
                    end
                    if i < number_of_domains
                        load_state!(solver, state)
                    end
