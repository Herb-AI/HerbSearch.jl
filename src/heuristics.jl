...
Defines a heuristic over holes, where the left-most hole always gets considered
first. Returns a [`HoleReference`](@ref) once a hole is found. This is the
default option for enumerators.
"""
function heuristic_leftmost(node::AbstractRuleNode, max_depth::Int)::Union{ExpandFailureReason,HoleReference}
    function leftmost(node::AbstractRuleNode, max_depth::Int, path::Vector{Int})::Union{ExpandFailureReason,HoleReference}
        if max_depth == 0
            return LimitReached()
        elseif isempty(HerbCore.get_children(node)) && !isterminal(node) && !allequal(get_grammar(node).childtypes[get_domain(node)])
            return HoleReference(node, path)
        else
            for (i, child) in enumerate(node.children)
                push!(path, i)
                hole_res = leftmost(child, max_depth - 1, path)
    ...
    # function leftmost(hole::AbstractHole, max_depth::Int, path::Vector{Int})::Union{ExpandFailureReason,HoleReference}
    #     if max_depth == 0
    #         return LimitReached()
    #     end
    ...
    return leftmost(node, max_depth, Int[])
end
