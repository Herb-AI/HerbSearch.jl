...
"""
    execute_angelic_on_input(symboltable::SymbolTable, program::RuleNode, grammar::AbstractGrammar, input::Dict{Symbol,Any}, 
        expected_output::Any, angelic_rulenode_idx::Int, max_attempts::Int, angelic_conditions::Dict{UInt16,UInt8})::Bool
...
- `angelic_rulenode_idx`: The index of the anglelic rulenode. It is used to replace angelified conditions.
...
function execute_angelic_on_input(
    ...
    visited = BitTrie()
    expr = create_angelic_expression(program, grammar, angelic_rulenode_idx, angelic_conditions)
...
"""
    create_angelic_expression(program::RuleNode, grammar::AbstractGrammar, angelic_rulenode_idx::Int, angelic_conditions::Dict{UInt16,UInt8})
    ...
    while !isempty(queue)
        ...
        for (child_index, child) in enumerate(node.children)
            if angelic_idx == child_index && child isa AbstractHole
                node.children[child_index] = RuleNode(angelic_rulenode_idx)
            else
                enqueue!(queue, child)
            end
        end
