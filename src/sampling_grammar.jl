...
"""
function Base.rand(
        ::Type{RuleNode}, grammar::AbstractGrammar, typ::Symbol, max_depth::Int = 10)
    dmap = mindepth_map(grammar)
...
"""
function Base.rand(
        ::Type{RuleNode}, grammar::AbstractGrammar, typ::Symbol, dmap::AbstractVector{Int},
        max_depth::Int = 10)
    rules = grammar[typ]
...
function _sample(
        node::RuleNode, typ::Symbol, grammar::AbstractGrammar, x::RuleNodeAndCount,
        maxdepth::Int)
    maxdepth < 1 && return
...
"""
function StatsBase.sample(::Type{NodeLoc}, root::RuleNode, typ::Symbol,
        grammar::AbstractGrammar, maxdepth::Int = typemax(Int))
    x = NodeLocAndCount(NodeLoc(root, 0), 0)
    if grammar.types[root.ind] == typ
        x.cnt += 1
    end
...
function _sample(::Type{NodeLoc}, node::RuleNode, typ::Symbol,
        grammar::AbstractGrammar, x::NodeLocAndCount, maxdepth::Int)
    maxdepth < 1 && return
    for (j, child) in enumerate(node.children)
