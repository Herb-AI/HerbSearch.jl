@testset verbose=true "Context-free iterators" begin
    ...
    @testset "bfs enumerator" begin
        ...
        programs = [freeze_state(p) for p in BFSIterator(g1, :Real, max_depth = 2)]
        @test all(map(t -> depth(t[1]) ≤ depth(t[2]),
            zip(programs[begin:(end - 1)], programs[(begin + 1):end])))
        ...
        @test all(p ∈ programs for p in answer_programs)
    end
