...
@testset verbose=false "Select compressions" begin
    ...
    @test HerbSearch.select_compressions(1, c, 0) == []
    @test HerbSearch.select_compressions(2, c, 1) == [ast_3, ast_2]
    @test HerbSearch.select_compressions(1, c, 1) == [ast_2, ast_3]
    @test HerbSearch.select_compressions(2, c, 0.5) == [ast_3]
    @test HerbSearch.select_compressions(1, c, 0.5) == [ast_2]
end
...
@testset verbose=false "Get compression size" begin
    ...
    @test HerbSearch.get_compression_size(subtree_dict, 2) == 3
    @test HerbSearch.get_compression_size(subtree_dict, 7) == 3
    @test HerbSearch.get_compression_size(subtree_dict, 1) == 0
end
...
@testset verbose=true "Generate Stats" begin
    ...
    @test HerbSearch.generate_stats(subtree_dict, c_ast) == c_info
end
