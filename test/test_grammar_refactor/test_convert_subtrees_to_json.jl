...
using .RefactorExt: convert_subtrees_to_json
...
@testset verbose=true "Parse Subtrees to JSON 1" begin
    ...
    @test convert_subtrees_to_json(dummy_subtrees, ast2) == desired_result
end

@testset verbose=true "Parse Subtrees to JSON 0 subtrees" begin
    desired_result = "{\"ast\":\"1\",\"subtrees\":[]}"
    @test convert_subtrees_to_json([], ast1) == desired_result 
end
...
@testset verbose=true "Parse Subtrees to JSON 1 subtree" begin
    ...
    @test convert_subtrees_to_json(dummy_subtrees, RuleNode(4, [RuleNode(1)])) == desired_result 
end