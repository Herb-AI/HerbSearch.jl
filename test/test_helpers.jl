...
function parametrized_test(argument_list, test_function::Function)
    ...
    argument_names = [String(arg) for arg in Base.method_argnames(method)[2:end]]
    ...
    @testset "parameterized test: $function_name_string" verbose=true begin
        for arguments in argument_list
            ...
            argument_pairs = ["$arg_name = $arg_value"
                              for (arg_name, arg_value) in zip(argument_names, arguments)]
...
function create_problem(f, range = 20)
    examples = [IOExample(Dict(:x => x), f(x)) for x in 1:range]
...
function test_constraints!(
        grammar::AbstractGrammar, constraints::Vector{<:AbstractGrammarConstraint};
    ...
    validtrees = 0
    for p in iter
        if all(check_tree(constraint, p) for constraint in constraints)
            validtrees += 1
        end
        alltrees += 1
    end
    ...
    constraint_iter = BFSIterator(
        grammar, starting_symbol, max_size = max_size, max_depth = max_depth)
...
function test_constraint!(grammar::AbstractGrammar, constraint::AbstractGrammarConstraint;
        max_size = typemax(Int), max_depth = typemax(Int), allow_trivial = false)
    test_constraints!(grammar, [constraint], max_size = max_size,
