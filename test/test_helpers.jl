...
"""
function test_constraints!(grammar::AbstractGrammar, constraints::Vector{<:AbstractGrammarConstraint}; max_size=typemax(Int), max_depth=typemax(Int), allow_trivial=false, iterator=BFSIterator)
    starting_symbol = grammar.types[1]
    iter = iterator(grammar, starting_symbol, max_size=max_size, max_depth=max_depth)
    alltrees = 0
    validtrees = 0
    for p ∈ iter
    ...
    constraint_iter = iterator(grammar, starting_symbol, max_size=max_size, max_depth=max_depth)
...
-  at least 1 program violates the constraint
"""
function test_constraint!(grammar::AbstractGrammar, constraint::AbstractGrammarConstraint; max_size=typemax(Int), max_depth=typemax(Int), allow_trivial=false, iterator=BFSIterator)
    test_constraints!(grammar, [constraint], max_size=max_size, max_depth=max_depth, allow_trivial=allow_trivial, iterator=iterator)
end
