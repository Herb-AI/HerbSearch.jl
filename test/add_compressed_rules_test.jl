@testitem "add_compressed_rules_test" setup=[HerbSearchSetup] begin
using Clingo_jll
using JSON
...
@testset "splitter_test" begin
    @testset "operation hole" begin
        ...
        splits = HerbSearch.split_hole(hole_rule, grammar)
    ...
    @testset "no split" begin
        ...
        splits = HerbSearch.split_hole(no_holes, grammar)
...
@testset "make_new_rules" begin
    @testset "leave same" begin
        no_holes = RuleNode(4, [RuleNode(1), RuleNode(2)])
        new_exprs = HerbSearch.create_new_exprs(no_holes, grammar, 1)
        @test only(new_exprs)[2] == :(Op = 1 + 2)
    end

    @testset "operation hole" begin
        hole_rule = UniformHole([0, 0, 0, 1, 1], [RuleNode(1), RuleNode(2)])
        new_exprs = HerbSearch.create_new_exprs(hole_rule, grammar, 1)
