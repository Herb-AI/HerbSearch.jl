@testitem "ASP Iterators" begin
    ...
    @testset "Forbid(1{a,a}) with 2-alias grammar" begin
        ...
        #     Const = 1 | 2
        # end
        # f = Forbidden(RuleNode(1, [VarNode(:a), VarNode(:a)]))
        # u = Unique(1)
        # addconstraint!(g1, f)
        # addconstraint!(g1, u)
        ...
        # @info "BFSASP", bfs_programs
        # @test false in bfs_programs
        # @test (@rulenode 1) in bfs_programs
        # @test (@rulenode 2) in bfs_programs
        # @test (@rulenode 4{5}) in bfs_programs
        # @test length(bfs_programs) == 3
    end
